    ...
    if respond_team_member_list and is_bot_msg:
        if sender_id:
            respond_in_thread(
                ...
                text="The VanguardAI Bot slash command is not enabled for this channel",
                thread_ts=None,
            )
    ...
    if answer is None:
        ...
        try:
            respond_in_thread(
                ...
                text="Hello! VanguardAI has some results for you!",
                blocks=[
                    SectionBlock(
                        text="VanguardAI is down for maintenance.\nWe're working hard on recharging the AI!"
                    )
                ],
                thread_ts=message_ts_to_respond_to,
