...
def make_assistant_private(
    ...
    if group_ids:
        raise NotImplementedError("enMedD AI MIT does not support private Assistants")
...
def get_assistants(
    ...
    include_default: bool = True,
    include_slack_bot_assistants: bool = False,
    include_deleted: bool = False,
    ...
    if not include_slack_bot_assistants:
        stmt = stmt.where(not_(Assistant.name.startswith(SLACK_BOT_PERSONA_PREFIX)))
    if not include_deleted:
...
def mark_delete_assistant_by_name(
    ...
    stmt = (
        update(Assistant)
        .where(Assistant.name == assistant_name, Assistant.builtin_assistant == is_default)
        .values(deleted=True)
    )
...
def upsert_assistant(
    ...
    if assistant:
        ...
        assistant = fetch_assistant_by_id(
            db_session=db_session, assistant_id=assistant.id, user=user, get_editable=True
        )
