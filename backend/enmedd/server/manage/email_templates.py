...
from enmedd.server.manage.models import EmailTemplates, SendMailTemplate
...
async def get_all_email_templates_route(workspace_id: int | None=None,
    _: User | None = Depends(current_workspace_admin_user),
    db_session: Session = Depends(get_session)) -> list[EmailTemplates]:

    current_workspace_admin_user()
    active_email_templates = get_active_email_template(db_session)
    if not active_email_templates:
        ...
        raise HTTPException(
            status_code=404,
            detail="No email templates found"
        )
...
async def send_sample_mail_route(
    ...
    if not send_sample_email(
        ...
        mailData.body, db_session):
        raise HTTPException(
            status_code=400,
            detail="Sending sample mail failed"
        )
