...
from enmedd.background.celery.celery_redis import celery_get_queue_length
...
from enmedd.db.connector import mark_ccpair_as_pruned
...
def check_for_vespa_sync_task(tenant_id: str | None) -> None:
    ...
    try:
        ...
        with get_session_with_tenant(tenant_id) as db_session:
            ...
            # check if any teamspaces are not synced
                ...
                try:
                    ...
                    for teamspace in teamspaces:
                        try_generate_teamspace_sync_tasks(
                            teamspace, db_session, r, lock_beat, tenant_id
                        )
...
def monitor_vespa_sync(self: Task, tenant_id: str | None) -> None:
    ...
    try:
        ...
        n_sync = celery_get_queue_length(
            EnmeddCeleryQueues.VESPA_METADATA_SYNC, r_celery
        )
        n_deletion = celery_get_queue_length(
            EnmeddCeleryQueues.CONNECTOR_DELETION, r_celery
        )
        n_pruning = celery_get_queue_length(
            EnmeddCeleryQueues.CONNECTOR_PRUNING, r_celery
        )
