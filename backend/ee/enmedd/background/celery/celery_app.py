...
from enmedd.background.update import get_all_tenant_ids
...
def check_sync_external_teamspace_permissions_task(tenant_id: str | None) -> None:
    """Runs periodically to sync external teamspace permissions"""
    with get_session_with_tenant(tenant_id) as db_session:
        cc_pairs = get_all_auto_sync_cc_pairs(db_session)
        for cc_pair in cc_pairs:
...
tasks_to_schedule = [
    ...
    {
        "name": "sync-external-teamspace-permissions",
