...
envsubst '$BACKEND_SERVER $DOMAIN $SSL_CERT_FILE_NAME $SSL_CERT_KEY_FILE_NAME' < "/etc/nginx/conf.d/$1" > /etc/nginx/conf.d/app.conf
...
while true; do
  ...
  if [ "$status_code" -eq 200 ]; then
    echo "API server: $BACKEND_SERVER responded with 200, starting nginx..."
    break  # Exit the loop
  else
    echo "API server: $BACKEND_SERVER responded with $status_code, retrying in 5 seconds..."
