...
$COMPOSE_CMD -f docker-compose.prod-api-only.yml run --name enmedd-stack --rm --entrypoint "\
...
$COMPOSE_CMD -f docker-compose.prod-api-only.yml -p enmedd-stack up --build --force-recreate -d nginx
echo
...
$COMPOSE_CMD -f docker-compose.prod-api-only.yml -p enmedd-stack up --build --force-recreate -d