...
export function AssistantEditor({
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      llm_model_version_override:
        existingAssistant?.llm_model_version_override ?? null,
      starter_messages:
        existingAssistant?.starter_messages?.map((message) => ({
          name: message.name ?? "",
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    let enabledTools = Object.keys(form.getValues("enabled_tools_map"))
      .map((toolId) => Number(toolId))
      .filter((toolId) => form.getValues("enabled_tools_map")[toolId]);
    const searchToolEnabled = searchTool
      ? enabledTools.includes(searchTool.id)
      : false;
    const imageGenerationToolEnabled = imageGenerationTool
                                  ...
                                  <div>
                                    <div className="flex flex-wrap gap-2 mt-2 mb-3 text-sm">
                                      {documentSets
                                        ...
                                        .map((documentSet) => {
                                          const ind = form
                                            .getValues("document_set_ids")
                                            ?.indexOf(documentSet.id);
