...
export function CCPairIndexingStatusTable({
  ...
  const removeFilterFromUrl = (key: string) => {
    const newParams = new URLSearchParams(window.location.search);
    newParams.delete(key); // Remove the parameter by its key
    const newUrl = `${window.location.pathname}?${newParams.toString()}`;
    window.history.pushState({}, "", newUrl); // Update the URL without reloading
  };

  const removeFilterFromLocalStorage = (key: string) => {
    localStorage.removeItem(key); // Remove filter from localStorage
  };
            ...
            <Badge>
              ...
              <X
                ...
                onClick={() => {
                  ...
                  removeFilterFromUrl("permissions");
